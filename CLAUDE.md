...
- **Linear integration (optional):** GraphQL API via httpx, API key auth (encrypted at rest)
...
React Frontend (Vite :5173)  ──/api proxy──>  FastAPI Backend (:8000)  ──>  PostgreSQL (:5432)
                                              ...
                                              GitHub REST API (read-only)
                                                     ↕
                                              Linear GraphQL API (read-only, optional)
                                                     ↕
                                              Claude API (on-demand AI analysis)
```

**Data flow:** GitHub App auth (JWT → installation token) → Scheduled sync fetches repos/PRs/reviews/issues → Webhooks for real-time updates → Stats service computes metrics on-demand → AI analysis optional. Linear sync (optional): API key auth → Scheduled sync fetches projects/cycles/issues → PR-issue linking by identifier regex → Sprint stats computed on-demand.
...
│   ├── integrations.py          # External integration config CRUD, sync trigger, user mapping, primary issue source (admin-only)
│   ├── sprints.py               # Sprint/cycle listing, velocity/completion/scope-creep trends, project portfolio, planning metrics (admin-only)
...
│   ├── encryption.py     # Shared Fernet encryption helpers for API keys and tokens (used by Slack + Linear)
│   ├── linear_sync.py    # Linear GraphQL client, sync orchestration (projects/cycles/issues), PR-issue linking, developer identity mapping
│   ├── sprint_stats.py   # Sprint metrics: velocity, completion rate, scope creep, triage, estimation accuracy, work alignment, planning-delivery correlation
...
| `integration_config` | External integration config (type, encrypted API key, workspace info, status, `is_primary_issue_source` toggle). Generic for Linear/future Jira. |
| `external_projects` | Linear projects with status, health (on_track/at_risk/off_track), progress, target date, lead FK |
| `external_sprints` | Linear cycles with state (active/closed/future), scope metrics (planned/completed/cancelled/added), team key |
| `external_issues` | Linear issues with identifier (e.g., ENG-123), status_category (triage/backlog/todo/in_progress/done/cancelled), priority 0-4, estimate, triage_duration_s, cycle_time_s |
| `developer_identity_map` | Maps developers to external system identities. UNIQUE(developer_id, integration_type). mapped_by: admin or auto (email match) |
| `pr_external_issue_links` | Links PRs to external issues. UNIQUE(pull_request_id, external_issue_id). link_source: title/branch/body/linear_auto |
...
- **`integration_config` is generic** — `type` column allows future Jira/etc. without schema changes. API key encrypted with shared Fernet (`services/encryption.py`), same `ENCRYPTION_KEY` as Slack.
- **Primary issue source toggle:** `is_primary_issue_source` flag on `integration_config`. Only one integration can be primary. `get_primary_issue_source()` in `linear_sync.py` returns `"github"` or `"linear"`. Issue stats functions branch on this to query `issues` or `external_issues` table.
- **External issue linking:** `pr_external_issue_links` stores PR↔Linear issue links found by regex `[A-Z]{2,10}-\d+` in PR title/branch/body. Processed in batches of 500 PRs. Unique constraint prevents duplicate links.
- **Linear sync uses `SyncEvent`** with `sync_type="linear"` for tracking. Runs on APScheduler interval (default 120 min via `LINEAR_SYNC_INTERVAL_MINUTES`). Background sync creates its own `AsyncSessionLocal()` session.
- **Developer identity mapping:** `developer_identity_map` has UNIQUE(developer_id, integration_type). Auto-mapping by email match during sync; admin can override via API. Unmapped users leave `assignee_developer_id` null on `external_issues`.
...
- **Encryption:** Shared Fernet encryption in `services/encryption.py` — `encrypt_token()`/`decrypt_token()` used by both Slack and Linear. `ENCRYPTION_KEY` env var required. Extracted from slack.py to avoid duplication.
- **Linear integration:** Optional. Admin configures via `POST /integrations` with API key (encrypted at rest). `LinearClient` in `linear_sync.py` is a read-only GraphQL client for Linear API. Sync orchestration: projects → cycles → issues → PR linking → developer auto-mapping. `SyncEvent(sync_type="linear")` tracks each run. Scheduled via APScheduler interval job (default 120 min, configurable via `LINEAR_SYNC_INTERVAL_MINUTES`). `extract_linear_keys()` regex `[A-Z]{2,10}-\d+` matches identifiers in PR title/branch/body. `get_primary_issue_source()` returns `"github"` or `"linear"` based on `is_primary_issue_source` flag. All existing features work unchanged when Linear is not configured.
- **Linear integration API:** `POST /integrations` (admin, create), `GET /integrations` (admin, list), `PATCH /integrations/{id}` (admin, update), `DELETE /integrations/{id}` (admin), `POST /integrations/{id}/test` (admin, test connection), `POST /integrations/{id}/sync` (admin, trigger sync), `GET /integrations/{id}/status` (admin, sync status + counts), `GET /integrations/{id}/users` (admin, list Linear users for mapping), `POST /integrations/{id}/map-user` (admin, map Linear user → developer), `GET /integrations/issue-source` (admin, current primary source), `PATCH /integrations/{id}/primary` (admin, set as primary issue source).
- **Sprint stats API:** `GET /sprints` (admin, list cycles with team/state filter), `GET /sprints/{id}` (admin, cycle detail with issues + metrics), `GET /sprints/velocity` (admin, velocity trend), `GET /sprints/completion` (admin, completion rate trend), `GET /sprints/scope-creep` (admin, scope creep trend), `GET /projects` (admin, project list with health + issue counts), `GET /projects/{id}` (admin, project detail with issues), `GET /planning/triage` (admin, triage queue metrics), `GET /planning/alignment` (admin, work alignment — linked vs unlinked PRs), `GET /planning/accuracy` (admin, estimation accuracy trend), `GET /planning/correlation` (admin, planning ↔ delivery correlation).
- **Sprint stats service:** `sprint_stats.py` computes: velocity trend (completed scope per cycle, half-split trend detection), completion rate (committed vs delivered %), scope creep (added mid-cycle as % of planned), triage metrics (avg/median/p90 triage duration, queue depth), estimation accuracy (estimated vs completed points per cycle), work alignment (% PRs linked to external issues), planning-delivery correlation (Pearson coefficient of completion rate vs avg merge time).
- **Slack integration:** Manual bot token setup (admin pastes xoxb- token). Bot token encrypted at rest using shared Fernet helpers from `services/encryption.py` (`ENCRYPTION_KEY` env var required). `get_decrypted_bot_token()` handles decrypt failures gracefully. `slack_config` singleton for global settings, `slack_user_settings` for per-developer DM preferences (Slack user ID + notification toggles). 6 notification types: stale_pr, high_risk_pr, workload, sync_complete, sync_failure, weekly_digest. DMs sent to individual developers via `slack_sdk` `AsyncWebClient`. Scheduled jobs run hourly and check configured hour at runtime. Post-sync hook sends sync notifications. `notification_log` tracks all sent messages.
